...
var transitionToLinePlots = function () {
    ...
    let cities = d3.selectAll(".plot");
    let new_h = ctx.h - 20;
    let offset = new_h / cities.size();

    cities
        .transition()
        .duration(1000)
        .attr("transform", (d, j) => `translate(0,${j * offset})`)

    d3.select("#timeAxis")
        .transition()
        .duration(1000)
        .attr("transform", `translate(0,${new_h})`);

    allLines = cities.selectAll("line")
        ...
        .duration(1000)
        .attr("transform", (d, j) => `translate(0,${cities.size() - d / 12})`)

        .transition()
        .duration(1000)
        .style("stroke", (d, j) => (d == null) ? "white" : "#4d4d4d");


};

var transitionToColorStrips = function () {
    allLines = d3.selectAll(".plot")

        .transition()
        .duration(1000)
        .style("stroke", (d) => (ctx.color(d*12)))
